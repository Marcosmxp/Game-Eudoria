...
constexpr std::array kFieldTexts{
    ...
    // The live reference shows the max-level value on the second line without
    // repeating the "Level:" label, so keep txtDeyMaxLevel hidden.
...
bool RoleCharacterHud::loadVisualManifest(
    ...
    while (std::getline(stream, line)) {
        ...
        try {
            ...
            // A malformed optional row should not prevent the rest of the exact
            // display list from loading.
        }
    }
